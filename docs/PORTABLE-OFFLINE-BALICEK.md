...
Cil: **jedna slozka** (aplikace + volitelne data) – uzivatel si ji **zkopiruje na lokálni disk** (`C:\...`) a spusti **`run_dashboard.bat`** nebo **`run_etl.bat`** – bez instalace Pythonu, bez `pip` z internetu. Sitovy disk muze slouzit jen k predani kopie; u velkych Parquetu je prace z lokálniho disku vyrazne prijemnejsi.
...
- Skripty **`run_dashboard.bat`** a **`run_etl.bat`** nejdriv hledaji `runtime\python\python.exe`, pokud chybi, pouziji variantu **`.venv`** (po `setup_venv.bat`).

**Vysledek pro uzivatele:** kopie cele slozky `KAPACITY` vcetne `runtime\` = spustitelna aplikace a ETL bez dalsich kroku (bez admin prav na instalaci Pythonu).

### Data (Parquet, `data_raw/`)

**Nejsou soucasti sestaveni** `build_portable.bat` – skript neinstaluje ani nekopiruje data, jen Python a knihovny.

- Parquet v kořeni projektu a soubory v `data_raw/` doplňte podle vaší distribuce (nebo je uživatel nahraje sám).
- Do Git repozitare se velke soubory typicky **nepushuji**; patri do firemni sdilene slozky nebo do ZIPu k aplikacnimu balicku.

**Doporuceny zpusob pro velka data:** datove soubory **dejte do stejne slozky jako aplikaci** a nechte uzivatele **kopii cele slozky rozbalit na lokálni disk** (SSD na `C:\...`), ne ji spoustet primo ze sitoveho disku.
...
| **Koncovi uzivatele** | Zkopiruji slozku (vcetne `runtime\` a dat), spusti `run_dashboard.bat` / `run_etl.bat`. |
...
1. Rozbalte / zkopirujte cely projekt `KAPACITY`.
2. Dvojklikem `build_portable.bat` **nebo** v CMD:
   ```bat
   cd /d C:\cesta\KAPACITY
   build_portable.bat
   ```
...
Po uspechu existuje `runtime\python\` (stovky MB kvuli Streamlitu, pandas, DuckDB atd.).
...
1. Zabalte **celou adresar** `KAPACITY` (vcetne `app.py`, `src\`, `runtime\`, `.bat`, dat…) do ZIP nebo zkopirujte na sitovy disk.
...
- **Streamlit** otevre prohlizec na `localhost`; aplikace bezi jen na danem PC.
